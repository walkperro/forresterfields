...
export default function GalleryGrid({ images }: { images: GalleryImage[] }) {
  ...
  return (
    <LazyMotion features={domAnimation}>
      <>
        <section className="mx-auto max-w-6xl px-4 pb-16 block">
          {/* 2–3 column masonry; smaller cards so ~3 rows show on load */}
          <m.div
            className="columns-2 md:columns-3 gap-3 [column-fill:_balance] block"
             variants={container}
            ...
            {filtered.map((img, i) => {
              ...
              return (
                <m.div
                  key={src + i}
                  variants={item}
                  className="inline-block align-top w-full mb-3 [break-inside:avoid] block"
                 initial="hidden" whileInView="show" viewport={{ once:false, amount:0.2 }}>
                  ...
                  >
                    <img src={src}
                      ...
                      width={900}
                      height={700}
                      className="w-full h-auto object-cover transition-transform duration-300 group-hover:scale-[1.02] block" style={{display:"block",width:"100%",height:"auto"}} loading="lazy" decoding="async" /></m.button>
                </m.div>
              );
            })}
          </m.div>
